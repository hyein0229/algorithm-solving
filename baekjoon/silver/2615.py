...
풀이 방법: 문제 자체는 이해가 어렵지 않지만 생각보다 구현하는 데 착각할 부분들이 있고 조금씩 계속 오류를 내서 시간이 오래 걸렸던 문제이다.
        처음에 착각한 부분은 가로, 세로, 대각선 방향을 상/하/좌/우/북동/북서 .. 이렇게 8가지 방향으로 나누어 따로 탐색했던 것이다.
        가로인 경우엔 좌우, 세로는 상하, 대각선인 경우엔 북동/남서 이렇게 같이 탐색을 해주어야 한다. 즉, 4가지로 나누어야 한다.
        그리고 가장 왼쪽에 있는 바둑알을 구할 때, for문을 왼쪽부터 탐색하므로 기준이 되는 x, y 가 답이 되겠다 생각했는데,
        남서쪽으로 가는 대각선의 경우엔 답이 되지 않는다. 그래서 연속된 바둑알을 배열에 넣고 열을 기준으로 정렬하여 구하였다. 
'''
# 가로, 세로, 두 방향의 대각선
